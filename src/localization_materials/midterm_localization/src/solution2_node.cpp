...
class Solution2Node{
public:
    Solution2Node(ros::NodeHandle nh, ros::NodeHandle pnh);
...
Solution2Node::Solution2Node(ros::NodeHandle nh, ros::NodeHandle pnh):
    nh_(nh), 
    pnh_(pnh), 
    cnt_gps_msgs_(0),
    cnt_imu_msgs_(0),
    ...
    ros::param::param<int>("~local_size", local_size_, 24);
    ros::param::param<int>("~submap_size", submap_size_, 30);
    ...
    sub_imu_ = nh_.subscribe("imu/data", 200, &Solution2Node::imu_cb, this);
    sub_trypose_ = nh_.subscribe("move_base_simple/goal", 1, &Solution2Node::pose_cb, this); 
    if(map_name_ == "nctu") { 
        ...
        sub_gnss_ = nh.subscribe("fix", 200, &Solution2Node::gnss_cb2, this);
        sub_pc_ = nh_.subscribe("lidar_points", 200, &Solution2Node::pc_cb, this);
    }
...
void Solution2Node::sigint_cb(int sig) {
    cout << "\nNode name: " << ros::this_node::getName() << " is shutdown." << endl;
    ros::shutdown();
}


void Solution2Node::filters_setup(double voxel_grid_size) {
...
void Solution2Node::map_setup(void) {
    // Find map pcd files
...
void Solution2Node::initial_guess(void) {
...
void Solution2Node::visualization_setup(void) {
    // GPS line strip
...
void Solution2Node::pc_cb(const sensor_msgs::PointCloud2ConstPtr &msg){
    ...
    pass.setFilterLimits(-local_size_ / 2, local_size_ / 2);
...
void Solution2Node::imu_cb(const sensor_msgs::ImuConstPtr &msg) {
    Eigen::Quaterniond q;
...
void Solution2Node::pose_cb(const geometry_msgs::PoseStampedPtr &msg) {
...
void Solution2Node::gnss_cb2(const geometry_msgs::PointStampedConstPtr &msg) {
...
void Solution2Node::publish_path_from_matrix(const Eigen::Matrix4d &m, const ros::Time &t, bool show_info) {
    ...
    if(show_info == true){
        ...
        Eigen::Vector3d euler = affine_matrix.rotation().eulerAngles(2, 1, 0); 
        cout << "yaw: " << euler(0) 
             << ", pitch: " << euler(1)
             << ", roll: " << euler(2) << endl;
        cout << "\n=============================================\n" << endl;
...
void Solution2Node::remove_plane(PointCloudXYZPtr cloud_in, PointCloudXYZPtr cloud_out) {
    // RANSAC remove plane 
...
void Solution2Node::extract_straight_objects(PointCloudXYZPtr cloud_in, PointCloudXYZPtr cloud_out, int num_threshold) {
    ...
    for (int i = 0; i < node.guess_list_.size(); i++) {
        ...
        if(index_todo != -1){
            ...
            for(int j = 0; j < node.guess_list_[index_todo].clouds_list_.size(); j++){
                ...
                quick_icp.setTransformationEpsilon(1e-12);
                quick_icp.setEuclideanFitnessEpsilon(1e-12); 
        ...
        if(node.flag_init_guess_ == false) {
            ...
            if(node.cnt_imu_msgs_ > 0 && node.cnt_gps_msgs_ > 0 
                ...
                for(int i = 0; i < kNumThreads; i++)
                    threads_list.push_back(std::thread(func, std::ref(node)));
            ...
            if(!node.clouds_list_.empty()) {
                ...
                // Remove plane
                // node.remove_plane(submap_pc, submap_pc);
